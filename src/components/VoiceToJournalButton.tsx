...
interface VoiceToJournalButtonProps {
  options: {
    moods: string[];
  };
...
export const VoiceToJournalButton: React.FC<VoiceToJournalButtonProps> = ({ options, onParsed }) => {
  ...
  const processAudio = async (audioBlob: Blob) => {
    ...
    reader.onloadend = async () => {
      try {
        ...
        const res = await authFetch('/api/voice-to-journal', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ audioBase64: base64Data, options })
        });
    ...
    <>
      ...
      {isError ? (
        ...
        <button
          type="button"
          onClick={startRecording}
          className="h-6 w-6 flex items-center justify-center bg-zinc-900 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 border border-zinc-800 rounded-lg transition-colors duration-200 ease-out shrink-0"
        >
          <Mic size={13} />
        </button>
      )}
