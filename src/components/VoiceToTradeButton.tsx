...
interface VoiceToTradeButtonProps {
  options: {
    platforms: string[];
    sessions: string[];
    concepts: string[];
...
export const VoiceToTradeButton: React.FC<VoiceToTradeButtonProps> = ({ options, onParsed }) => {
  ...
  const processAudio = async (audioBlob: Blob) => {
    ...
    reader.onloadend = async () => {
      try {
        ...
        const res = await authFetch('/api/voice-to-trade', {
          method: 'POST',
        ...
        if (!parsedData || (!parsedData.asset && !parsedData.notes && !parsedData.type && !parsedData.pnl && !parsedData.status)) {
